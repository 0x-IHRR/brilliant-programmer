...
人工教学/真实模型准确性、跨语言全面覆盖、历史规模性能尚未验收。最初 checkpoint 的迁移 0026 接 0024；现已正常合并 #29 并接到 0025，保持真实主线单 head，没有 stamp。没有部署。
...
两项旧失败随后在唯一 `--basetemp` 下、原断言与原 15 秒限制单次复验：2 通过，10.61 秒。前端 P2 修复的真实旧构建两红（切换 B 后 A 迟到跳转、前置链接丢原项目身份）；新构建最终完整 harness 退出 0，2 浏览器用例通过 9.3 秒，包含响应丢失恢复、A 后台原记录保留、A 未保存编辑跨项目切换保留，精确 8 次请求与 1 个路线分析任务。中间 UI 已绿但 harness 仍失败：第二个受控响应重复 age=60 被生产去重拒绝；改为源码支持的 age=61 异常情境后才完整通过，没有放宽生产去重或计数。

旧主题浏览器 3 通过（14.9 秒），先生成真实正式原答，再实际 0024→0026 升级：7 表共 8 行完整 JSON 一致，四张项目事实表各 UPDATE/DELETE 共 8 个不可变触发器检查通过。保留 schema `issue23_migration_509932f0`。第一次迁移脚本因尚无 free_topic 正式原答在选样本时报 NoResultFound，未将其记作成功。评分区视口截图为 `project-feedback-viewport-320.png`；模型质量仍未认证。

整合 #29 后，保留原 public schema 的实验分叉与线上证据，运行环境转到同一专用数据库的新空 schema `issue23_integrated_29`，从零实际迁移至 0026（经 0025）并初始化合成管理员。另用旧正式记录验证 0024→0025→0026，7 表 8 行仍相等、8 个不可变检查仍通过，保留验证 schema `issue23_migration_4482fe89`。

最新整合定向验证 23 项通过（33.56 秒），覆盖项目普通路线/错误与停止恢复/Key 轮换秘密出站、独立帮助及完整 70 条历史协议（含旧 free_topic/JD），并实际导入同来源的受控 failed 质量报告：普通项目练习仍发布；其新独立检验返回 409、未创建新轮、零新增模型请求，原题保留。使用 `--basetemp=/tmp/bp-issue-23-pytest-integrated-20260910-01`。ruff、strict mypy 116 个模块与最新前端构建通过；完整浏览器按 CI 顺序串行进行，最终结果另补。
