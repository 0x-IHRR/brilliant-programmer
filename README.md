...
- 每个尝试从 DNS 到完整响应最长 120 秒，响应体累计最多 1 MiB，拒绝压缩响应以避免解压膨胀；JSON/SSE均受限。最多 3 次尝试，重试间隔 1 秒、2 秒。只重试临时 DNS 失败、连接失败、超时、429 及 500/502/503/504；TLS、非法目标、跳转、401/403、402、明确 `insufficient_quota`/`billing_hard_limit_reached`/`insufficient_balance`、格式或协议错误不自动重试。错误原文不返回、不写应用日志；供应商错误码仅用于上述分类，不能假定所有供应商采用同一额度错误格式。
- `attempts` 保留本次各次尝试序号/分类与供应商 `prompt_tokens/completion_tokens/total_tokens`，缺失、失败或无效数字分别为 `null`（显示未知），不估金额、不把未知补零，也不设置累计 token/任务额度。它是本次响应统计，刷新后不保留，不是账单或后续持久训练任务的汇总。持久任务预算、结果、防重及重启恢复由 T07 #8 实现，不能把这两个短操作当后台训练任务。
