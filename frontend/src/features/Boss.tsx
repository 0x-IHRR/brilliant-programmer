...
export function BossProgress({ runId, onLevel }: { runId: string; onLevel?: (level: string) => void }) {
  ...
  return <section aria-label="Boss结算与晋升" className="space-y-3 border p-3"><h3>本轮 Boss 结算</h3><Button className={cls} variant="outline" onClick={() => setRefresh(n => n + 1)}>重新读取Boss结论</Button>{error && <p role="alert">{error}</p>}{value && <><p>服务器当前等级：{value.current_level} · 累计修为：{value.points}</p>{value.promotion_id && <p role="status">本轮晋升已保存：{value.stage.from_level} → {value.stage.to_level}；重复读取或提交不再升级。</p>}{value.disposition === "blocked_quality" && <p role="status">本轮评分质量未达标，已冻结为不新增独立证明或晋升；已有等级与修为保留，复测后需主动新开挑战。</p>}{value.disposition === "blocked_revalidation" && <p role="status">本轮晋升处置已冻结为不晋升：发起或结算时存在待补验。以后解除限制也不会把本轮旧回执补作晋升；请主动开始新挑战。</p>}{value.revalidation_of && <p>本轮为关联补验，发起时真实等级：{value.launch_level}；通过只解除所关联要求，不再升级。</p>}{value.disposition === "revalidated" && <p>本轮补验通过事实已保存；该晋升的当前要求与后续复核历史如下。</p>}{value.revalidation?.promotion_id && <RevalidationHistory item={value.revalidation} />}{value.decision ? <p>{value.disposition === "blocked_quality" ? "这次评分仅供练习与复盘，不作为独立通过依据。" : label(value.decision.outcome)}</p> : <p>尚未形成冻结的有效评分；未晋升，不表示挑战失败。</p>}{value.decision?.shortfalls?.map(item => <div key={item.judgment_id}><p>{item.judgment_id}：{item.gap}</p><a className="underline" href={`/?capability=${encodeURIComponent(item.target.capability_id)}&difficulty=${encodeURIComponent(item.target.difficulty)}`}>针对 {item.target.capability_id} 补练或主动检验</a></div>)}<p>本轮全部判断与理由完成仍沿原轮一次10点；不会因本次失败扣修为或撤销历史。语义质量尚未验收。</p><a className="underline" href="/?boss=1">返回Boss入口，查看标准或主动新题重试</a></>}</section>
}
