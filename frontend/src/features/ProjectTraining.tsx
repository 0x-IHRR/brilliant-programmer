...
import { ProjecttrainingService, TopicsService, type Goal, type ModelConfigPublic, type ProjectTrainingPublic, type UnitAccess, type EvidenceKey } from "../client"
import { Button } from "../components/ui/button"

const cls = "h-auto min-h-9 max-w-full whitespace-normal"
export function ProjectTraining({ projectRunId, config }: { projectRunId: string; config: ModelConfigPublic | null }) {
  ...
  async function analyze() {
    ...
    const key = JSON.stringify([projectRunId, item?.topic.id, version?.id, config.version])
    if (pending.current?.key !== key) pending.current = { key, id: crypto.randomUUID(), topic: item?.topic.id ?? crypto.randomUUID() }
    const request = pending.current
    const { data } = await ProjecttrainingService.analyzeRoute({ body: { request_id: request.id, topic_id: request.topic, project_run_id: projectRunId, expected_version: version?.id ?? null, expected_config_version: config.version, disclosure_accepted: true } })
    if (current()) { install(data); setSelected(data.topic.id) }
  }
  async function start(nodeId: string) {
    if (dirty) { setError(unsaved); return }
    if (!version || !item || !config?.version || config.revoked || !accepted) return
    const key = JSON.stringify([version.id, nodeId, config.version])
  ...
  return <section aria-label="项目模块学习路线" className="space-y-3 border p-3 break-words">
    ...
    {relevant.length > 1 && <label>已有路线<select value={item?.topic.id ?? ""} disabled={busy} onChange={e => setSelected(e.target.value)}>{relevant.map(r => <option key={r.topic.id} value={r.topic.id}>{r.current?.repository.commit ?? r.topic.id}</option>)}</select></label>}
    {config?.version && !config.revoked ? <><p className="break-all">模型接收方：{config.service_url} · {config.model_id}</p><label className="flex items-start gap-2"><input type="checkbox" aria-label="允许发送项目模块片段" checked={accepted} onChange={e => setAccepted(e.target.checked)} /><span>路线分析与核对发送已读模块片段和能力目录；开题只发送所选目标、模块与冻结必要来源。每项任务最多六次模型调用，重试可能计费。</span></label></> : <p>旧路线和题目可继续查看；新调用需要保存可用配置。</p>}
    ...
    {item?.topic.jobs.map(job => <div key={job.id}><p role="status">{job.message} · 已记录 {job.attempts} 次调用</p>{["queued", "running", "stopping", "failed", "stopped"].includes(job.status) && <Button className={cls} disabled={busy} onClick={() => void act(async () => { await TopicsService.jobAction({ path: { topic_id: item.topic.id, job_id: job.id, action: ["failed", "stopped"].includes(job.status) ? "retry" : "stop" } }); await refresh(item.topic.id) })}>{["failed", "stopped"].includes(job.status) ? "恢复原路线分析剩余预算" : "停止路线分析"}</Button>}</div>)}
    {item?.current && <><p className="break-all">固定版本：{item.current.repository.owner}/{item.current.repository.name}@{item.current.repository.commit}</p><p>{version?.message}</p><details><summary>本次范围与缺项</summary>{item.current.map_missing.map((m, i) => <p key={i}>{m}</p>)}</details></>}
