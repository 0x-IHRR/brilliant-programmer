          ...
          return <details key={capability.id} className="space-y-2">
            ...
            {proof && <details><summary>查看状态依据（{(proof.history ?? []).length} 条原答）</summary>
              {(proof.history ?? []).map(item => <article key={item.evidence.original_id} className="border p-2 space-y-1 break-all">
                ...
                {item.evidence.review_decision && <p>评分复核：{({ pending: "待复核", upheld: "维持原判", corrected: "原判已更正", disputed: "仍有争议" } as Record<string, string>)[item.evidence.review_decision]}（按原作答位置回算）</p>}
                <p>结果：{({ independent_pass_candidate: "有效独立通过候选", evidenced_fail: "有据失败", practice: "普通练习", unclear: "结论含糊", pending_delivery: "帮助交付待核实", system_failure: "系统失败", invalid_case: "无效案例", no_qualified_case: "无合格陌生案例", awaiting_evaluation: "等待有效评分", evidence_unavailable: "证据不可用", disputed: "未决争议", pending: "待复核", quality_failed: "评分质量未达标，本轮不作为新增独立证据" } as Record<string, string>)[item.evidence.outcome] ?? "待核实"}；{item.counted ? "计入此能力、难度与背景" : "未增加或打断连续次数"}；此时连续 {item.streak}，状态：{item.status === "verified" ? "已验证" : item.status === "needs_consolidation" ? "待巩固" : "未验证"}</p>
                <p>本次评分依据：{item.evidence.grading_quality === "passed" ? "报告范围内达标（不代表教学效果已证实）" : item.evidence.grading_quality === "failed" ? "已知未达标" : item.evidence.grading_quality === "version_mismatch" ? "版本不匹配，可靠性未验证" : "评分可靠性未验证"}；{item.evidence.quality_report_id ? `报告 ${item.evidence.quality_report_id}` : "尚无适用报告"}。</p>
