...
from tests.service_data_benchmarks import comparison, load
...
def test_eighteen_independent_source_and_evidence_paths(tmp_path, provider, draft):
    _, auth = account()
...
def test_six_ordinary_errors_preserve_round_award(tmp_path, provider, draft):
    _, auth = account()
    config = save(auth, service_url=provider["url"]).json()
    configure(provider, draft)
...
def test_three_domains_seen_rename_and_directional_delivery(tmp_path, provider, draft):
    _, auth = account()
    config = save(auth, service_url=provider["url"]).json()
    configure(provider, draft)
