...
from tests.test_request_benchmark_api import BASIC, configure, launch
from tests.test_training import provider, start_worker, stop_worker, wait_run

artifacts = Path("/tmp/bp-issue-35-artifacts")
artifacts.mkdir(exist_ok=True)
for draft in BASIC[::2]:
    with tempfile.TemporaryDirectory(prefix="request-benchmark-browser-") as directory:
        path = Path(directory)
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "../frontend",
                    "test",
                    "request-benchmarks.spec.ts",
                ],
                env={
                    **os.environ,
                    "REQUEST_BENCHMARK_TOKEN": auth["Authorization"].removeprefix(
                        "Bearer "
                    ),
                    "REQUEST_BENCHMARK_ORIGIN": identity,
                    "REQUEST_BENCHMARK_DRAFT": draft.model_dump_json(),
                },
                check=True,
            )
