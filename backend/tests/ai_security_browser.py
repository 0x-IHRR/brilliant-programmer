"""Two declared basic-domain UI paths, not all tiers or human quality approval."""
...
from tests.test_ai_security_api import BASIC, configure, launch
...
artifacts = Path("/tmp/bp-issue-39-artifacts")
artifacts.mkdir(exist_ok=True)
for draft in BASIC[::2]:
    with tempfile.TemporaryDirectory(prefix="ai-security-browser-") as directory:
        path = Path(directory)
        ...
        try:
            ...
            subprocess.run(
                ...
                env={
                    **os.environ,
                    "AI_SECURITY_TOKEN": auth["Authorization"].removeprefix("Bearer "),
                    "AI_SECURITY_ORIGIN": identity,
                    "AI_SECURITY_DRAFT": draft.model_dump_json(),
                },
                check=True,
            )
