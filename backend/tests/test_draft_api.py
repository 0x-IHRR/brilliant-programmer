...
def test_draft_partial_replay_owner_and_no_business_facts(ready, provider):
    ...
    result = client.put(url(run_id), headers=auth, json=body)
    assert result.status_code == 200, result.text
    assert result.headers["cache-control"] == "no-store"
    saved = result.json()
    assert saved["progress"] == body["progress"]
    assert client.put(url(run_id), headers=auth, json=body).json() == saved
    assert client.get(url(run_id), headers=auth).json() == saved
    assert client.put(url(run_id), headers=auth, json=draft()).status_code == 409
    changed = {**body, "progress": {**body["progress"], "step": "coach"}}
    assert client.put(url(run_id), headers=auth, json=changed).status_code == 409
    _, other = account()
    assert client.get(url(run_id), headers=other).status_code == 404
    assert client.put(url(run_id), headers=other, json=body).status_code == 404
...
def test_concurrent_same_version_has_one_winner(ready):
    ...
    first = client.put(url(run_id), headers=auth, json=draft()).json()
    barrier = Barrier(2)

    def save(step):
        barrier.wait(timeout=5)
        return client.put(
            url(run_id), headers=auth, json=draft(first["version"], step=step)
        )
    ...
    assert client.get(url(run_id), headers=auth).json() == winner
...
def test_formal_lineage_blocks_old_editor_without_overwriting_original(ready):
    ...
    response = client.put(url(run_id), headers=auth, json=update)
...
def test_save_and_submit_have_one_lineage_order(ready, monkeypatch, draft_first):
    ...
    def save():
        return client.put(url(run_id), headers=auth, json=draft())
