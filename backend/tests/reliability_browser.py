...
from tests.test_reliability_api import BASIC, configure, launch
from tests.test_training import provider, start_worker, stop_worker, wait_run

artifacts = Path("/tmp/bp-issue-37-artifacts")
artifacts.mkdir(exist_ok=True)
for draft in BASIC[::2]:
    with tempfile.TemporaryDirectory(prefix="reliability-browser-") as directory:
        path = Path(directory)
        ...
        try:
            ...
            subprocess.run(
                ...
                env={
                    **os.environ,
                    "RELIABILITY_TOKEN": auth["Authorization"].removeprefix("Bearer "),
                    "RELIABILITY_ORIGIN": identity,
                    "RELIABILITY_DRAFT": draft.model_dump_json(),
                },
                check=True,
            )
