...
from tests.test_engineering_api import BASIC, configure, launch
...
artifacts = Path("/tmp/bp-issue-38-artifacts")
artifacts.mkdir(exist_ok=True)
for draft in BASIC[::2]:
    with tempfile.TemporaryDirectory(prefix="engineering-browser-") as directory:
        path = Path(directory)
        ...
        try:
            ...
            subprocess.run(
                ...
                env={
                    **os.environ,
                    "ENGINEERING_TOKEN": auth["Authorization"].removeprefix("Bearer "),
                    "ENGINEERING_ORIGIN": identity,
                    "ENGINEERING_DRAFT": draft.model_dump_json(),
                },
                check=True,
            )
