...
async def generate(service_url: str, model_id: str, key: str, target: EvidenceKey, sources: list[Source], correction: bool) -> tuple[str, dict[str, int | None]]:
    payload = json.dumps({
        ...
        "messages": [
            ...
            {"role": "user", "content": json.dumps({
                "target": target.model_dump(), "catalog_version": CATALOG.version,
                "sources": [source.model_dump() for source in sources],
                "schema": Candidate.model_json_schema(),
                "correction": "前一候选未通过校验，请重新核对完整字段、逐字引用与一致性" if correction else None,
            }, ensure_ascii=False)},
        ],
    ...
    try:
        if content_type == "application/json":
            ...
            for event in raw.decode().replace("\r\n", "\n").split("\n\n"):
                ...
                for choice in json.loads(data)["choices"]:
                    if choice.get("index") == 0 and (part := choice["delta"].get("content")) is not None:
        ...
        raise ProbeError("invalid_response", "模型未返回可校验候选", counts=counts) from None
